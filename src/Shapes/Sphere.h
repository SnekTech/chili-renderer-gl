...
public:
    ...
    static IndexedTriangleList<V> GetPlain(float radius = 1.0f, int latDiv = 12, int longDiv = 24)
    {
        ...
        const float latitudeAngle = PI / (float)latDiv;
        ...
        {
            const auto latBase = base * Mat3::RotationX(latitudeAngle * (float)iLat);
            for (int iLong = 0; iLong < longDiv; iLong++)
            {
                vertices.emplace_back();
                vertices.back().pos = latBase * Mat3::RotationZ(longitudeAngle * (float)iLong);
            }
        }
